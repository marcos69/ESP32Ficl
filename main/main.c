...
#define DEVICE_IP          "192.168.178.33"
...
static void ficlsTask(void *data) {
	testFicl();
  vTaskDelete(NULL);
}
...
static esp_err_t wifiEventHandler(void *ctx, system_event_t *event)
{
	if (event->event_id == SYSTEM_EVENT_STA_GOT_IP) {
		xTaskCreatePinnedToCore(&telnetTask, "telnetTask", 4048, NULL, 5, NULL, 0);
	}
  return ESP_OK;
}
...
int app_main(void)
{
  ...
  //init();
  xTaskCreate(&ficlsTask, "ficlsTask", 4*1024, NULL, 5, NULL);
	return 0;
}
